...
export default function AllowCommentsToggle({ slug }: { slug: string }) {
  ...
  const [value, setValue] = React.useState<boolean>(allTrue("allowComments"))

  React.useEffect(() => { setValue(allTrue("allowComments")) }, [allTrue])

  const apply = async (v: boolean) => {
    try {
      queryClient.setQueryData(["feedback-boards", slug], (prev: any) => {
        const arr = Array.isArray(prev) ? prev : []
        return arr.map((it: any) => (it.slug !== "roadmap" && it.slug !== "changelog") ? { ...it, allowComments: v } : it)
      })
    } catch {}
    try {
      const res = await client.board.updateGlobalSettings.$post({ slug, patch: { allowComments: v } })
      if (!res.ok) {
        const err = (await res.json().catch(() => null)) as { message?: string } | null
        throw new Error(err?.message || "Update failed")
      }
      await refetch()
      toast.success("Comments setting updated")
    } catch (e: unknown) {
      await refetch()
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="rounded-md border bg-card p-3 flex items-center justify-between">
        ...
        <Switch checked={value} onCheckedChange={(v) => { setValue(v); apply(v) }} aria-label="Allow Comments" />
      </div>
    </div>
  )
}
