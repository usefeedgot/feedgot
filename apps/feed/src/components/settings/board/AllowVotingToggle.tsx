...
export default function AllowVotingToggle({ slug }: { slug: string }) {
  ...
  const [value, setValue] = React.useState<boolean>(allTrue("allowVoting"))

  React.useEffect(() => { setValue(allTrue("allowVoting")) }, [allTrue])

  const apply = async (v: boolean) => {
    try {
      queryClient.setQueryData(["feedback-boards", slug], (prev: any) => {
        const arr = Array.isArray(prev) ? prev : []
        return arr.map((it: any) => (it.slug !== "roadmap" && it.slug !== "changelog") ? { ...it, allowVoting: v } : it)
      })
    } catch {}
    try {
      const res = await client.board.updateGlobalSettings.$post({ slug, patch: { allowVoting: v } })
      if (!res.ok) {
        const err = (await res.json().catch(() => null)) as { message?: string } | null
        throw new Error(err?.message || "Update failed")
      }
      await refetch()
      toast.success("Voting setting updated")
    } catch (e: unknown) {
      await refetch()
  ...
  return (
    <div className="space-y-2">
      <div className="text-md font-medium">Allow Voting</div>
      <div className="text-sm text-accent">Enable upvotes on feedback posts.</div>
      <div className="rounded-md border bg-card p-3 flex items-center justify-between">
        <div className="text-sm">Enable voting</div>
        <Switch checked={value} onCheckedChange={(v) => { setValue(v); apply(v) }} aria-label="Allow Voting" />
      </div>
    </div>
  )
}
